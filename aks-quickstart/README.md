...
## Introduction
How long does it take to configure your first AKS development environment, sidecarless Service Mesh, and GitOps workflow?

How about 15 minutes? Give us that much time and we’ll give you an ephemeral testbed to explore the benefits of the new architecture mode in Istio, deploy a few applications, and validate zero-trust with zero-effort! We’ll host all of this on a AKS cluster to keep the setup standalone and as simple as possible.
...
## Prerequisites
For this exercise, we’re going to do all the work on a AKS cluster. All you’ll need to get started is CLI utilities kubectl, Azure CLI, and curl. Make sure these are all available to you before jumping into the next section. I’m building this on MacOS but other platforms should be perfectly fine as well.
...
```
AZURE_RESOURCE_GROUP="myproject"
AKS_CLUSTER_NAME="aks-quickstart"
MAIN_MACHINE_TYPE="Standard_DS3_v2"
MAIN_NUM_NODES="1"
```

Create the cluster using `az` CLI
```bash
az aks create --resource-group ${AZURE_RESOURCE_GROUP} --name ${AKS_CLUSTER_NAME} --node-count ${MAIN_NUM_NODES} --node-vm-size ${MAIN_MACHINE_TYPE} --generate-ssh-keys
```

Use the following command to get the kubeconfig once the cluster has been created
```bash
...
If you’d like to cleanup the work you’ve done, simply delete the AKS cluster where you’ve been working.
```bash
az aks delete --resource-group ${AZURE_RESOURCE_GROUP} --name ${AKS_CLUSTER_NAME}
```

## Learn More
In this blog post, we explored how you can get started with Istio Ambient and Argo CD on your own workstation. We walked step-by-step through the process of standing up a AKS cluster, configuring the new Istio Ambient architecture, installing a couple applications, and then validating zero trust for service-to-service communication without injecting sidecars! All of the code used in this guide is available on github.
